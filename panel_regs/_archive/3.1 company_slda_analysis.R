...
rel_topics <- rel_topics[,2:(wordnum+1)]
...
rel_text <- rep("None", topicnumber)
for (i in seq(1,topicnumber,1)){
  if(as.numeric(as.character(rel_topics[(2*i),(wordnum)])) > 0){
    for(j in seq(1,wordnum,1)){
      h <- 2*i
      count <- as.numeric(as.character(rel_topics[h,j]))
      word <- toString(rel_topics[h-1,j])
      words <- toString(rep(word,round((5000*count),0)))
      rel_text[i] <-  paste(rel_text[i], words)
    }
  }
}
for (i in seq(1,topicnumber)){
  x <- paste0(figure_location, "/30topic_slda/company_rel_slda_30topic", (i-1),"_cloud.pdf")
  pdf(x)
  wordcloud(rel_text[i], scale=c(6,1),max.words=wordnum, random.order=FALSE, 
            rot.per=0.4, colors = brewer.pal(8, "Blues"),vfont=c("serif","plain"))
  #line <- readline() # 
  dev.off()
}
...
# Relative sLDA phi
phi_filename <- paste0(source_location, "/company_rel_slda_30phi.csv")
phi <- read.csv(phi_filename)
phi <- phi[1:30,] # Adjust here to cut out the unused iterations
phi_long <- melt(phi, id = "X")

ggplot(data=phi_long, aes(x=X, y=value, colour=variable)) +
  geom_line() + theme(legend.position="none") + labs(x = "EM iterations", 
    y = expression(paste("Values of ", phi)))
export_filename <- paste0(figure_location, "/company_rel_30phi_converge.png")
ggsave(export_filename, width = 8, height = 4, dpi = 200)
