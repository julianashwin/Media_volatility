...
test <- felm(abs_intra_day ~ mention | Code, data = cross_firm_news)
summary(test)
test = felm(highlow ~ plm::lag(highlow_control_shock, 0:3) + plm::lag(own_sec_news_highlow_c_notme, 0:3) + 
...
temp_data_abs_c <- cross_firm_news
temp_data_highlow_c <- cross_firm_news
temp_data_mention <- cross_firm_news
temp_data_mention_avg <- cross_firm_news
...
test <- felm(abs_intra_day ~ mention + abs_shock | Code, data = cross_firm_news)
summary(test)



temp_data <- pdata.frame(temp_data, index = c("Code", "Date"))
temp_data_mention <- pdata.frame(temp_data_mention, index = c("Code", "Date"))
temp_data_mention_avg <- pdata.frame(temp_data_mention_avg, index = c("Code", "Date"))
...
clean_dir <- "~/Documents/DPhil/Clean_Data"
clean_filename = paste(clean_dir, "FT/matched/each_sector_news_data_NACE.csv", sep = "/")
write.csv(temp_data, file = clean_filename, row.names = FALSE)
# temp_data <- read.csv(clean_filename, stringsAsFactors = FALSE)
clean_dir <- "~/Documents/DPhil/Clean_Data"
clean_filename = paste(clean_dir, "FT/matched/each_sector_mention_data_NACE.csv", sep = "/")
...
clean_dir <- "~/Documents/DPhil/Clean_Data"
clean_filename = paste(clean_dir, "FT/matched/each_sector_mention_avg_data_NACE.csv", sep = "/")
...
start_col <- which(colnames(temp_data) == "Sector01_abs_c_shock")
end_col <- which(colnames(temp_data) == "Sector97_abs_c_shock")
...
test <- felm(abs_intra_day ~ abs_control_shock | Code , data = temp_data)
summary(test)
test <- felm(abs_intra_day ~ mention + Sector66_mention | Code , data = temp_data_mention)
summary(test)
test <- felm(highlow ~ abs_control_shock + Sector66_highlow_c_shock| Code  , data = temp_data)
summary(test)
test <- felm(highlow ~  Sector66_highlow_c_shock| Code + Date, data = temp_data)
summary(test)

table(temp_data$Sector06_and_07_abs_c_shock != 0 & temp_data_mention$Sector06_and_07_mention != 0)
table(temp_data_mention$Sector == "06_and_07")
table(temp_data$Sector06_and_07_abs_c_shock != 0 & temp_data_mention$Sector == "06_and_07")
table(temp_data$Sector06_and_07_abs_c_shock != 0 & temp_data_mention$Sector == "06_and_07" & 
        temp_data$own_sec_news_abs_c == temp_data$Sector06_and_07_abs_c_shock)
...
which(colnames(temp_data_mention) == "Sector01_mention")
