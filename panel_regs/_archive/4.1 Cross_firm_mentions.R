...
clean_filename = paste(clean_dir, "FT/matched/all_equityprices_companyarticles1.csv", sep = "/")
...
test <- felm(highlow ~ mention | Code + Date, data = cross_firm_news)
summary(test)
test <- plm(highlow ~ mention + abs_intra_day + 
              plm::lag(highlow, 1:10) + VI_put, data = cross_firm_news, 
