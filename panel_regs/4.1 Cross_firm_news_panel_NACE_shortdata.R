...
clean_dir <- "~/Documents/DPhil/Clean_Data"
import_filename = paste(clean_dir, "FT/matched/standardised_data.csv", sep = "/")
all_data <- read.csv(import_filename, stringsAsFactors = FALSE)
all_data$Date <- as.Date(all_data$Date)

all_data <- all_data[,c("Code", "Date", "VI_put")]

all_data <- merge(all_data, cross_firm_news, by = c("Code", "Date"), all.x = TRUE)
all_data <- pdata.frame(all_data, index = c("Code", "period"))
...
summary(test)
test = felm(highlow ~ plm::lag(highlow_control_shock_phi, 0) + plm::lag(own_sec_news_highlow_c_notme, 0) + 
                  plm::lag(own_sec_highlow_notme, 0) + 
